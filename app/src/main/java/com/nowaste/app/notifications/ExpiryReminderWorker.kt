...
class ExpiryReminderWorker(
    ...
    private fun sendReminder(item: FoodItem, today: LocalDate, nearExpiryDays: Int) {
        ...
        val title = when (item.expiryDate) {
            today.plusDays(nearExpiryDays.toLong()) -> applicationContext.getString(
                R.string.notification_title_near_expiry,
                nearExpiryDays,
            )
            today -> applicationContext.getString(R.string.notification_title_today)
            today.minusDays(1) -> applicationContext.getString(R.string.notification_title_expired_yesterday)
            else -> return
        }
